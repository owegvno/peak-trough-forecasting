...
def write_wide_rows(dataset_dir: Path, target_col: str, rows: list[dict[str, object]], pred_days: int = 2) -> None:
    ...
    fieldnames.extend([f"{target_col}_过去96小时_均值", f"{target_col}_历史峰值_均值4天"])
    path = dataset_dir / target_col / f"峰谷预测样本_seq48_pred48_{target_col}.csv"
...
def make_records(start: datetime, hours: int) -> list[dict[str, str]]:
    ...
    for i in range(hours):
        ...
        row = {"date": current.strftime("%Y/%-m/%-d %H:%M")}
        hour = current.hour
...
class DatasetGenerationTests(unittest.TestCase):
    ...
    def test_convert_wide_dataset_to_long_table_merges_all_variable_features(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((dataset_dir / "长表" / "峰谷预测长表_seq48_pred48_全部变量.csv").exists())
            ...
            self.assertEqual(rows[0]["target_col"], "OT")
            self.assertEqual(rows[0]["target_peak_value"], "31.0")
            self.assertEqual(rows[1]["target_peak_hour"], "14")
