...
def build_dataset_with_selected_baselines(
    ...
    """Build one residual-learning row per sample with selected value/hour baselines."""
    ...
    dataset = value_output.merge(hour_output, on=list(SAMPLE_KEY_COLUMNS), how="inner", validate="one_to_one")
    if len(dataset) != len(value_output):
        raise ValueError(f"合并后的残差学习数据行数不完整：期望 {len(value_output)} 行，实际 {len(dataset)} 行")
    ...
    dataset = dataset.loc[:, output_columns].sort_values(list(SAMPLE_KEY_COLUMNS)).reset_index(drop=True)
